// Drives the REAL compiled macOS application end-to-end, specifically to
// verify the macOS native-engine packaging fix: the release bundle now
// embeds libdupora_engine.dylib into Contents/Frameworks/ (see the
// "Bundle Framework" copy-files build phase in
// macos/Runner.xcodeproj/project.pbxproj) and
// dupora_native_bindings.dart resolves it there explicitly. This test
// proves that end-to-end rather than assuming it from the Xcode-project
// change alone - real dlopen, real BLAKE3 hashing, real duplicate
// detection against files this test creates itself, real cleanup
// afterward.
//
...
void main() {
  ...
  testWidgets('macOS release bundle: native engine loads and hashes for real', (
    tester,
  ) async {
    // --- Step 1: prove the native engine loads, independent of any UI. ---
    // If Contents/Frameworks/libdupora_engine.dylib were missing or
    // unresolved, this throws a StateError from DuporaNativeBindings
    // ._openLibrary() immediately - no scan, no widget tree, nothing to
    // mask it.
    ...
    final testDir = await Directory.systemTemp.createTemp('dupora_macos_it_');
    addTearDown(() async {
    ...
    const sharedContent = 'Dupora macOS integration test duplicate content.';
    ...
    await tester.pump();
    // Don't hard-assert an intermediate scanning screen: with only 3 tiny
    // files, the scan can complete faster than a single pump() observes
    // on a loaded CI runner, going straight to results (see the identical
    // reasoning/fix already applied to linux_native_engine_test.dart).
    ...
    // ignore: avoid_print
    print(
      'macOS native-engine runtime verification passed: engine loaded, '
