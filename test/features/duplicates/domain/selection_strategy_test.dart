...
ScannedFile _file(String path, {required DateTime modified}) {
  return ScannedFile(path: path, name: path, extension: '', size: 100, modifiedAt: modified);
}
...
void main() {
  ...
  group('applySmartSelection', () {
    test('keepOldest keeps the earliest modified file', () {
      ...
      final group = DuplicateGroup(fullHashHex: 'h', fileSize: 100, files: [newest, oldest]);

      final result = applySmartSelection(group, SmartSelectionStrategy.keepOldest, protectedLocations);
    ...
    test('keepNewest keeps the latest modified file', () {
      final oldest = _file('old', modified: DateTime(2020));
      final newest = _file('new', modified: DateTime(2024));
      final group = DuplicateGroup(fullHashHex: 'h', fileSize: 100, files: [oldest, newest]);

      final result = applySmartSelection(group, SmartSelectionStrategy.keepNewest, protectedLocations);
    ...
    test('keepShortestPath keeps the file with the shortest path', () {
      ...
      final group = DuplicateGroup(fullHashHex: 'h', fileSize: 100, files: [long, short]);

      final result = applySmartSelection(group, SmartSelectionStrategy.keepShortestPath, protectedLocations);
    ...
    test('the kept file is never present in selectedForDeletion', () {
      ...
      final group = DuplicateGroup(fullHashHex: 'h', fileSize: 100, files: files);
      final result = applySmartSelection(group, SmartSelectionStrategy.keepOldest, protectedLocations);
      expect(result.selectedForDeletion, isNot(contains(result.keep)));
      expect(result.selectedForDeletion, hasLength(2));
    });

    test('a protected duplicate is never selected for deletion even if not the keep file', () {
      final protected = ProtectedLocations(userDefined: const ['C:\\Protected']);
      final normal = _file('C:\\Users\\me\\a.txt', modified: DateTime(2024));
      final inProtected = _file('C:\\Protected\\b.txt', modified: DateTime(2020));
      final group = DuplicateGroup(fullHashHex: 'h', fileSize: 100, files: [normal, inProtected]);
      ...
      final result = applySmartSelection(group, SmartSelectionStrategy.keepNewest, protected);
  ...
  group('selectAllDuplicates', () {
    test('selects everything but the keep file, honoring protection', () {
      ...
      final protectedDup = _file('C:\\Protected\\c.txt', modified: DateTime(2024));
      final group = DuplicateGroup(fullHashHex: 'h', fileSize: 100, files: [keep, normalDup, protectedDup]);
